...
# Load the dataset
df <- read_csv('airfare_ml_dataset.csv')
...
ggplot(df, aes(x = Fare)) +
  ...
  labs(title = "Distribution of Price", x = "Fare", y = "Frequency")

# Boxplot of Price vs. Airline
ggplot(df, aes(x = Airline, y = Fare)) +
  geom_boxplot(fill = "#0ea830", color = "#302828") +
  labs(title = "Boxplot of Price vs. Airline", x = "Airline", y = "Fare")
...
df <- df %>%
  select(-c('Date_of_journey', 'Flight_code','Class','Departure','Arrival','Duration_in_hours','Days_left'))

# Split the data into features and target
X <- df %>%
  select(-Fare)  # 'Fare' is the target variable
y <- df$Fare
...
train_data <- cbind(X_train, Fare = y_train)

# Linear Regression
linear_reg <- lm(Fare ~ ., data = train_data)

# Decision Tree
tree_reg <- rpart(Fare ~ ., data = train_data)
...
# Random Forest
rf_reg <- randomForest(Fare ~ ., data = train_data)
