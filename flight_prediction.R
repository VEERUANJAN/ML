...
# Load the dataset
df <- read_csv('flight_price_data.csv')
...
categorical_columns <- c('airline','source_city', 'stops', 'destination_city')
for (col in categorical_columns) {
  df[[col]] <- as.numeric(factor(df[[col]]))
}

# Drop unnecessary columns
# Remove the first column if unnecessary
df <- df[, -1]
df <- df %>%
  select(-c('flight', 'departure_time','arrival_time','class','duration','days_left'))
